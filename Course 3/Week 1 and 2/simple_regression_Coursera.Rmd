...
n1 <- lm(runs~homeruns, data=mlb11)
summary(m1)
```
...
713-(-2789.2426 + 0.6305 * 5579) 
mlb11[mlb11$at_bats == 5579, "runs"] - predict(m1, new.at_bats)
```

The command mlb11[mlb11$at_bats == 5579, "runs"] called out the value of variable runs when at_bats gets 5579
...
# checking again the linearity with residual plot and checking for normal residual distribution and homoscedascity
## Create residual
q8 <- lm(runs~hits, data=mlb11)
## Draw residual plot with intercept line when y = 0
ggplot(data = q8, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")
## Draw QQ plot
ggplot(data = q8, aes(sample = .resid)) +
  stat_qq()
```

The relationship between hits and runs seems like to be a linear relationship.
...
```{r runs-vs-new-vars}
# Using R^2 to compare. The graphs can be draw with similar code as above
