...
summary(m_bty_gen)
```
bty_avg still a significant predictor of score with p value less than 0.05. 
The addition of 'gender' to the model changed the parameter estimate for 'bty_avg'.
Beta value changed from 0.06664 to 0.07416.
...
summary(m_bty_rank)
```
The category 'Teaching' alphabetically comes first, so it is chosen as reference group with highest predicted course evaluation score (looking at the beta).  
...
9. Which of the following is the correct interpetation of the coefficient associated with the ethnicity variable. <br> Non-minority professors are expected on average to score ... 
<ol>
...
```
The coefficients and significance of the other explanatory variables didn't change much. 
It means the 'cls_profs' maybe not collinear with other explain variables. But the variables
being left in the model are having collinearity.
...
m <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
#final model using backward selection by AIC
step(m, direction = "backward", trace=TRUE )
```
...
Yes, if each row is considered as a course, then the data is not independent. Because, the information of 
one course can be repeated.
...
No, we can not yield our result to other population. Because, our sample only including 
professors in Uni of Texas at Austin, which is not representing for the whole University 
professors population.
