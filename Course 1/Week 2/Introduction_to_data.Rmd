...
ggplot(data = sfo_feb_flights, aes(x=arr_delay))+
  geom_histogram(binwidth = 150)
sfo_feb_flights %>% 
  summarise(median_arr_delay = median(arr_delay),range_arr_delay = range(arr_delay))
...
sfo_feb_flights %>% 
  group_by(carrier) %>%
  summarise(median_arr_delay = median(arr_delay), IQR_arr_delay = IQR(arr_delay)) %>% arrange(desc(IQR_arr_delay))
...
nycflights %>% ggplot(aes(y = distance, x = avg_speed))+
  geom_point()
...
```{r on-time-arr-perc}
nycflights_q10 <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"), dep_type = ifelse(dep_delay < 5, "on time dep", "delayed dep"))
table(nycflights_q10$arr_type, nycflights_q10$dep_type)
nycflights_q10 %>% group_by(dep_type)%>%
